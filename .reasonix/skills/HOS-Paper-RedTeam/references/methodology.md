...
**无全文 → 仅方向性垫背，不精确引用。** 原则：背书只证明「这个方向是真实存在的、领域公认的」，不冒充「书里具体怎么写的」。遵守以下铁律：

1. **无全文，只能方向性引用**：未读全文的书（本 Skill 中除 How to Read a Paper 外全部），只允许说「该书的定位是 X（官方目录/简介证实）」，**禁止**引用具体页码、具体论证细节、具体措辞。缺全文时宁可少背书，不可假背书。
2. **每条背书必须带锚点**：ISBN / DOI / 官方出版社 URL 三选一，禁止只写书名就声称「官方介绍明确说明……」。锚点需亲自访问核验（访问方式见 `references/authoritative-sources.md` §三）。
3. **区分「已核验」与「待核验」**：下表 `✅ 已核验（<日期>）` = 官方页面/DOI 已实际访问确认；任何未亲自核验的声明一律标注 `⚠ 待人工核验`，禁止假装查证过。
4. **禁止转述二手来源当背书**：别人（含 AI）对书的总结、书单推荐、二手书评不构成背书。背书只认出版社官方页面 / DOI 记录 / 作者原文。
5. **引用页码/章节必须真实**：引用的章节号必须来自官方目录（见下表「目录核实」列），引不到就写「章节号待核实」，禁止编造。
6. 发现背书错误（如 ISBN 不存在）→ 立即修正并更新 CHANGELOG，向用户说明。
...
| ★★★★☆ | **Adrian Wallwork, *English for Writing Research Papers*, 3rd ed., Springer, 2023** | 国际论文写作 | DOI 10.1007/978-3-031-31072-0 · ISBN 9783031310720 · ✅ Crossref + 官方前言已核验 2026-02-16 | 硕/博 |
