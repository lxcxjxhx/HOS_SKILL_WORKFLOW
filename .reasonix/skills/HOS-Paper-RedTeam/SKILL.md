...
7. **评分卡永远在最顶部**——用 TS 渲染器（`node src/index.ts <review.json>`）生成 ghfind 式评分卡（分数+评级+维度条+一句话点评），长文靠后。**默认输出完整审计报表 HTML**（`src/report.ts`）：评分卡头部 + **问题统计面板** + **逐条根因证据链卡片**；`--card` 输出纯评分卡 HTML，`--format md` 输出 markdown 卡。评分模型见 [score-model.md](references/score-model.md)。
8. **每条 RVE 必须携带根因物证（tex 原文引用）**——`issues[]` 中每条 `texQuotes[]` 必须是从 tex 源码**逐字复制**的牵涉段落（含文件+行号），禁止改写或凭记忆转述；查不到 tex 源码（如仅 PDF 的论文）写「无 tex 源码可引用，以 PDF 原文为准」并挂 RVE-REPRO。
9. **问题统计由渲染器自动计算**——`stats`（total / bySeverity / byCategory）从 `issues[]` 派生，无需手填；Scorer 只需保证 `issues[]` 完整。
10. 输出语言跟随用户语言；默认中文。
...
## 五·五、根因证据链与问题统计（HTML 审计报表规范）

完整审计报表（`src/report.ts`）要求每条 RVE 以 `EvidenceLink` 结构登记在 `issues[]` 中，构成**可追溯的证据链路**：

```
Claim（论文声称，含位置 claimRef）
  → tex 原文（texQuotes[]：file + line + 逐字复制的 tex 代码 + note）
  → 证据缺口（gap：声称与证据差在哪）
  → RVE 判定（rveId + rootCause 根因 + exploit 影响）
  → Patch（修复方案）
  → 外部佐证（evidence：arXiv/OpenReview/PWC/S2 锚点）
```
...
| 字段 | 必填 | 说明 |
|------|------|------|
| `rveId` / `category` / `severity` | ✅ | 沿用 HOS-RVE 编号系统 |
| `title` | ✅ | 问题标题，一句话点破根因 |
| `claim` / `claimRef` | ✅ | 论文声称 + 声称位置（摘要/§N/Table N）|
| `rootCause` | ✅ | 根因：为什么这个声称站不住（详细解释）|
| `texQuotes[]` | ✅ | **逐字复制**的 tex 原文段落（file+line+code+note），根因直接物证 |
| `gap` | ✅ | 声称与证据之间的缺口 |
| `exploit` | ✅ | 影响：洞会被怎么利用/误导 |
| `patch` | ✅ | 修复方案 |
| `evidence` | 可选 | 外部权威佐证锚点 |

铁律：
1. `texQuotes[].code` 必须**逐字复制**自 tex 源码（`assets/tex/<N>-<论文>/` 或 arXiv e-print），禁止改写、删减数字、凭记忆转述。
2. 无 tex 源码的论文（如 SAST-Genius 仅 PDF）：`texQuotes` 留空并在 `evidence` 注明「无 tex 源码，以 PDF 原文为准」（RVE-REPRO）。
3. `stats`（问题统计：total / bySeverity / byCategory）**由渲染器从 issues[] 自动计算**，手填会被覆盖——保证 issues 完整即可。
4. 渲染：`node src/index.ts <review.json>` → 完整审计报表 HTML（评分卡 + 统计面板 + 根因证据链）。
...
本技能产出的 Research Gap 数据库可直接喂给 S-07-HOS-IP-Writing 的论文写作管线，实现"审别人的论文 → 挖自己的论文"闭环。
