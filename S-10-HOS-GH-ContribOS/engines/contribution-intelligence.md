...
## 2.1 项目评估体系（8 维度 + 加权评分）

基于 41 个 PR 复盘 + 全量回查（第三方 PR 合并率仅约 9%）提炼的评估标准。**合并速度是过审率的最强预测因子，已纳入第 8 维。**

| 维度 | 权重 | 评估方法 | 合格标准 |
|------|------|---------|---------|
| 技术栈匹配度 | 15% | 检查主要语言/框架/依赖/构建工具 | 日常使用的技术栈 |
| 社区活跃度 | 10% | `git log --since="3 months ago"` + 近期 commit/issue/PR | 近 3 月有持续提交 |
| 贡献友好度 | 15% | 检查 CONTRIBUTING.md / PR 模板 / good first issue | 有明确贡献指南 |
| 维护者响应 | 10% | 历史 PR 的平均首次回复时间 | < 1 周 |
| 文档完善度 | 5% | README + API 文档 + 架构说明 | 有完整开发文档 |
| 测试覆盖率 | 10% | 是否有 CI + 测试是否容易本地运行 | 有自动化测试 |
| 改动规模适配 | 10% | 是否有 small scope 的改进空间 | 有 good first issue 标签 |
| **合并速度** | 25% | 近期 PR 平均合并时长 + 是否有非成员 PR 被合并 | 平均 ≤ 3 天 且 有 ≥1 外部合并 |

**合并速度测量**：
```bash
gh pr list --repo <owner>/<repo> --state merged --limit 10 --json mergedAt,createdAt
gh pr list --repo <owner>/<repo> --state all --search "MCP OR AI OR agent"  # 维护者疲劳/拒绝信号
```

**红旗信号**（任一命中即降级该仓库）：
- 合并速度平均 > 7 天，或近期无外部（非成员）PR 被合并
- 近 1 个月出现"拒绝 AI / 批量 / typo / 小文档"类维护者公告
- 首次贡献者 PR 全部被关
...
| 门 | 核心问题 | 红灯信号 |
|----|---------|---------|
| **G0 问题真实性**（修复类必过） | 反证搜索：资源是否已释放？缓冲区是否有硬性上界？是否已有同类 PR/修复？问题是否仍在最新代码复现？改动含实际代码修复吗？ | grep 命中就当问题存在；无法给出复现；只改文档 |
...
| **G6 重复与方向**（大功能必过） | 同仓库是否已有同类功能/PR？issue 是否有"不要提交"声明？大功能是否已先开 issue 获认可？ | 新功能复制粘贴多仓库；已有同类；无 issue 归属 |
| **G7 项目规则**（微小改动必过） | CONTRIBUTING + 近 1 月公告是否拒绝 typo/小文档/格式改动？有替代贡献通道吗？ | 撞"不接受此类"规则仍提交 |
| **G8 批量节流**（多 PR 必过） | 本周期提交数、同仓库同模式累计数、是否同模板复制？ | 1 天 ≥4 仓库；同仓库同模式 ≥3；同模板多仓库 |
...
| "grep 到泄漏/溢出，肯定是 bug" | G0 | 做反证搜索：资源谁释放？缓冲区上界？已被修过？ |
| "给这些项目都加个功能很有用" | G6/G8 | 逐仓库查同类功能 + 先开 issue；同模板多仓库 = 红灯 |
| "改动小到不值得查规则" | G7 | 查 CONTRIBUTING + 近 1 月公告是否拒此类 |
| "多提交几个总能中一个" | G8 | 批量节流；宁可少提交高质量 |

**触发词**（命中任一 → 强制检索场景记忆并过对应闸门）：
- 语义类：`self-join`、`窗口函数`、`period-over-period`、`环比`、`指标计算`、`语义等价`、`构造转换`、`internal representation`、`LAG`
- 修复类：`resource leak`、`buffer overflow`、`stack overflow`、`VLA`、`类型断言`、`空指针`、`false positive`
- 功能类：`MCP`、`AI Agent`、`新功能`、`大功能`、`批量提交`、`同一功能多仓库`
- 重复类：`duplicate`、`已有PR`、`typo`、`微小改动`、`文档小修`

**批量节流硬规则**：1 天 ≤3 仓库；同一功能模板禁止复制到多仓库；同仓库同模式累计 ≥3 即暂停该模式（经验数据：第三方 PR 合并率仅约 9%，批量提交不会提高过审率）。
