...
每条记录包含字段：日期 / 项目 / PR链接 / 状态 / 技术领域 / **发生了什么** / **维护者原话（verbatim，逐字，勿转述）** / **根因分析** / **应由哪道闸门拦截（G1–G5）** / **提炼规则（编号可复用）** / **检索关键词**。
...
- 触发词（命中任一强制检索）：`self-join`、`窗口函数`、`period-over-period`、`环比`、`指标计算`、`语义等价`、`构造转换`、`internal representation`、`LAG`。
