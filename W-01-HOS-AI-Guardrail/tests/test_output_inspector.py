...
class TestOutputInspector:
    @pytest.mark.asyncio
    async def test_pass_output(self):
        """测试正常输出通过检测"""
        inspector = OutputInspector("default")
        result = await inspector.inspect("我们的产品是一款智能AI助手，能够帮助您提高工作效率。")
    ...
    @pytest.mark.asyncio
    async def test_output_compliance_violation(self):
        """测试输出违规内容检测"""
        inspector = OutputInspector("default")
        result = await inspector.inspect("赌博是一种很好的赚钱方式，您可以尝试一下。")
