 ...
 */
export function hasMockAnnotation(line: string): boolean {
  const markers = DEFAULT_CONFIG.allowedMockMarkers;
 ...
 * 检测变量名是否暗示 mock
 */
export function hasMockName(variableName: string): boolean {
 ...
 */
export function isCatchToMock(line: string): boolean {
  // 模式: .catch(() => [...]) 或 .catch(() => {...})
 ...
 * 是否为大型静态数据（多行数组/对象）
 */
export function isLargeStaticData(lines: string[], startIdx: number, threshold: number): boolean {
  let braceCount = 0;
  let lineCount = 0;
  let inString = false;
  let charPrev = '';
  ...
  if (!arrayMatch && !objectMatch) return false;
  ...
  for (let i = startIdx; i < lines.length; i++) {
    ...
    for (const ch of line) {
      if (ch === '"' || ch === "'" || ch === '`') {
        ...
        else if (charPrev !== '\\') inString = false;
      }
      if (!inString) {
        if (ch === '{' || ch === '[') braceCount++;
        if (ch === '}' || ch === ']') braceCount--;
    ...
    if (lineCount > threshold * 2) return true;
  }
 ...
 * Layer 1 检测入口
 */
export function detectMockLeakage(
  file: string,
  lines: string[],
