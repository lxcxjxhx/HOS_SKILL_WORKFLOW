...
exports.printMarkdownReport = printMarkdownReport;
const RESET = '\x1b[0m';
const RED = '\x1b[31m';
const GREEN = '\x1b[32m';
 ...
 * 格式化风险等级
 */
function formatRisk(level, value) {
 ...
 * 控制台报告
 */
function printReport(result, format = 'pretty') {
    if (format === 'json') {
    ...
    if (result.findings.length > 0) {
        ...
        for (const f of result.findings) {
            ...
            if (f.snippet)
                console.log(`       Code: ${DIM}${f.snippet}${RESET}`);
            if (f.suggestion)
                console.log(`       Fix:  ${CYAN}${f.suggestion}${RESET}`);
            console.log();
        }
    }
    else {
