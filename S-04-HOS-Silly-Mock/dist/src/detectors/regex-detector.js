...
exports.containsRegexLiteral = containsRegexLiteral;
exports.containsNewRegExp = containsNewRegExp;
exports.containsRegexMethod = containsRegexMethod;
exports.isStructuralContext = isStructuralContext;
exports.isHighRiskRegex = isHighRiskRegex;
exports.suggestReplacement = suggestReplacement;
exports.detectRegexAbuse = detectRegexAbuse;
 ...
 * 检查行是否包含 regex 字面量
 */
function containsRegexLiteral(line) {
 ...
 * 检查行是否在结构化解析上下文中
 */
function isStructuralContext(line, prevLines, nextLines) {
    const context = [
        line.toLowerCase(),
        ...prevLines.slice(-3).map(l => l.toLowerCase()),
        ...nextLines.slice(0, 2).map(l => l.toLowerCase()),
    ].join(' ');
    return STRUCTURED_CONTEXT_INDICATORS.some(indicator => context.includes(indicator.toLowerCase()));
}
/**
 * 检查 regex 模式是否高风险
 */
function isHighRiskRegex(line) {
    return HIGH_RISK_REGEX_PATTERNS.some(pattern => pattern.test(line));
}
/**
 * 根据上下文推荐替代方案
 */
function suggestReplacement(line, context) {
 ...
 * Layer 2 检测入口
 */
function detectRegexAbuse(file, lines, config = types_1.DEFAULT_CONFIG) {
    const findings = [];
    const inStringStack = [];
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (isJson && !options.checkJsonParsing)
            continue;
        if (isHtml && !options.checkHtmlParsing)
            continue;
        if (isXml && !options.checkXmlParsing)
            continue;
        if (isUrl && !options.checkUrlParsing)
            continue;
        // 评估严重程度
