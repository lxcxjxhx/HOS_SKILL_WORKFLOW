...
name: HOS-Sec-Engine
description: >
  Combined code audit + pentest rules
...
| Penetration Test Rules | 7 | PT-001 ~ PT-007 |
...
**Severity**: Medium | **CWE**: CWE-200, CWE-209, CWE-215 | **OWASP**: A01:2021 - Broken Access Control, A05:2021 - Security Misconfiguration
...
**Check Flow**:
   - **Error Message Exposure Detection**: Check if error messages or stack traces are exposed to end users
   - **HTTP Header Information Leakage**: Check if HTTP headers expose technology stack details
   - **API Documentation Exposure**: Check if API documentation endpoints are accessible without authentication
   - **Source Code and Configuration Exposure**: Check if sensitive files are accessible via web server
   - **Debug Mode and Verbose Logging**: Check if debug mode or verbose logging is enabled in production

**Evidence Requirements**:
   - [Required] Location where information exposure occurs in code
   - [Required] Server and application configuration that controls information exposure
   - [Required] HTTP response headers and error responses that reveal information

**Remediation**:
   - [High] Implement centralized error handling that sanitizes error messages (Difficulty: Easy)
   - [Medium] Remove or obfuscate identifying HTTP headers (Difficulty: Easy)
   - [High] Protect API documentation and debug endpoints (Difficulty: Easy)
   - [High] Configure web server to block access to sensitive files (Difficulty: Easy)

**Pentest Validation**:
How to validate information exposure during penetration testing
  - Attack Steps: Send a request to a non-existent endpoint to trigger error handling: GET /api/nonexistent
  ...
  - Tools: curl -v https://target/api/endpoint (inspect headers), gobuster dir -u https://target -w common.txt (directory brute force), whatweb https://target (technology fingerprinting), Nuclei templates for information disclosure detection
...
**Severity**: Critical | **CWE**: CWE-287, CWE-307, CWE-384, CWE-798 | **OWASP**: A07:2021 - Identification and Authentication Failures
...
**Check Flow**:
   - **JWT Weak Secret Detection**: Check if JWT signing secret is weak or hard-coded
   - **JWT Algorithm Confusion**: Check if JWT algorithm can be manipulated by attacker
   - **Session Fixation Vulnerability**: Check if session ID is regenerated after authentication
   - **Brute Force and Credential Stuffing**: Check if login endpoint has rate limiting and account lockout
   - **Password Reset Logic Flaws**: Check if password reset flow has logic vulnerabilities
   - **Multi-Factor Authentication Bypass**: Check if MFA can be bypassed or skipped

**Evidence Requirements**:
   - [Required] Authentication code location and implementation details
   - [Required] Authentication configuration including token expiry, rate limits, session settings
   - [Required] HTTP request/response evidence of authentication behavior

**Remediation**:
   - [Critical] Use cryptographically strong JWT secrets from key management (Difficulty: Easy)
   - [Critical] Enforce algorithm whitelist in JWT verification (Difficulty: Easy)
   - [High] Regenerate session ID after authentication (Difficulty: Easy)
   - [High] Implement rate limiting and account lockout (Difficulty: Easy)

**Pentest Validation**:
How to validate authentication bypass during penetration testing
  - Attack Steps: Attempt login with common credentials: admin/admin, admin/password, root/root
  ...
  - Tools: jwt_tool.py -T jwt_token -C wordlist.txt (JWT brute force), hashcat -m 16500 for JWT secret cracking, Burp Suite Intruder for credential stuffing and brute force, curl -X POST /login -d "username=admin&password=test" (manual testing)
...
**Severity**: High | **CWE**: CWE-269, CWE-285, CWE-639, CWE-862 | **OWASP**: A01:2021 - Broken Access Control
...
**Check Flow**:
   - **Object-Level Access Control (IDOR)**: Check if objects are accessed by user-controllable ID without ownership verification
   - **Vertical Privilege Escalation**: Check if role-based access controls can be bypassed to access admin functionality
   - **Role Parameter Manipulation**: Check if role or permission fields can be modified by the user in requests
   - **Horizontal Privilege Escalation**: Check if users can access or modify data belonging to other users at the same privilege level
   - **Authorization Middleware Placement**: Check if authorization checks are consistently applied across all protected endpoints

**Evidence Requirements**:
   - [Required] Authorization code location and access control implementation
   - [Required] API endpoint definitions and access control requirements
   - [Required] HTTP request/response showing privilege escalation success

**Remediation**:
   - [Critical] Implement object-level authorization for all data access (Difficulty: Medium)
   - [Critical] Apply authorization middleware consistently to all protected routes (Difficulty: Easy)
   - [High] Implement field-level filtering to prevent mass assignment of role fields (Difficulty: Easy)

**Pentest Validation**:
How to validate privilege escalation during penetration testing
  - Attack Steps: Authenticate as a regular user and enumerate accessible endpoints
  ...
  - Tools: Burp Suite AuthMatrix extension for authorization testing, Autorize Burp plugin for automatic authorization bypass detection, curl -H "Authorization: Bearer <user_token>" https://target/api/admin/users, Postman collection with different user tokens for automated testing
...
**Severity**: High | **CWE**: CWE-362, CWE-840, CWE-841, CWE-682 | **OWASP**: A04:2021 - Insecure Design, A01:2021 - Broken Access Control
...
**Check Flow**:
   - **Race Condition Detection**: Check if concurrent requests can cause inconsistent state or double operations
   - **Negative Amount/Quantity Manipulation**: Check if negative values can be used to exploit financial or inventory logic
   - **State Machine Bypass**: Check if workflow states can be skipped or manipulated out of order
   - **Coupon and Discount Abuse**: Check if coupon/discount logic can be exploited for unintended benefits
   - **Precision and Rounding Exploitation**: Check if floating-point precision issues can be exploited
   - **Time-Based Logic Manipulation**: Check if time-dependent logic can be manipulated by changing request timing

**Evidence Requirements**:
   - [Required] Business logic code location with financial/state operations
   - [Required] Data flow for financial operations and state changes
   - [Required] HTTP request/response showing business logic exploitation

**Remediation**:
   - [Critical] Use database transactions with proper isolation for financial operations (Difficulty: Medium)
   - [High] Implement idempotency keys for financial operations (Difficulty: Medium)
   - [High] Implement state machine validation for workflow transitions (Difficulty: Medium)

**Pentest Validation**:
How to validate business logic flaws during penetration testing
  - Attack Steps: Race condition testing: Send 10 concurrent requests to apply the same coupon or transfer funds
  ...
  - Tools: Turbo Intruder (Burp extension) for race condition testing, curl with xargs for concurrent request testing, Postman Runner for workflow step bypass testing, Custom Python scripts for precision testing
...
**Severity**: High | **CWE**: CWE-915, CWE-20, CWE-770, CWE-307 | **OWASP**: A04:2021 - Insecure Design, A08:2021 - Software and Data Integrity Failures
...
**Check Flow**:
   - **Mass Assignment Detection**: Check if request body is directly passed to model creation or update without field filtering
   - **Response Data Over-Exposure**: Check if API responses include more data than necessary
   - **Rate Limit Bypass**: Check if rate limiting can be bypassed through various techniques
   - **GraphQL Abuse**: Check if GraphQL endpoints are vulnerable to query complexity attacks
   - **Pagination Abuse**: Check if pagination can be abused to extract excessive data
   - **Parameter Pollution**: Check if duplicate or conflicting parameters can cause unexpected behavior

**Evidence Requirements**:
   - [Required] API endpoint code with request handling and response generation
   - [Required] API endpoint definitions, request/response schemas, and rate limit configuration
   - [Required] HTTP request/response showing API abuse success

**Remediation**:
   - [Critical] Implement strict field whitelisting for all input operations (Difficulty: Easy)
   - [High] Implement response DTOs to filter output data (Difficulty: Easy)
   - [Medium] Implement comprehensive rate limiting with multiple identifiers (Difficulty: Easy)
   - [Medium] Add GraphQL security configuration (Difficulty: Medium)

**Pentest Validation**:
How to validate API abuse during penetration testing
  - Attack Steps: Mass assignment testing: Send POST /api/users with {"role":"admin","isAdmin":true} in addition to normal fields
  ...
  - Tools: curl -X POST /api/users -H "Content-Type: application/json" -d '{"name":"test","role":"admin"}', GraphQL Voyager for schema visualization, InQL Burp extension for GraphQL security testing, Custom scripts for parameter pollution testing
...
**Severity**: High | **CWE**: CWE-79, CWE-601, CWE-352, CWE-97 | **OWASP**: A03:2021 - Injection, A01:2021 - Broken Access Control
...
**Check Flow**:
   - **Stored and Reflected XSS Detection**: Check if user input is rendered in HTML/JS without proper escaping
   - **Open Redirect Vulnerability**: Check if redirect URLs are derived from user input without validation
   - **CSRF Protection**: Check if state-changing operations are protected against CSRF attacks
   - **Email/SMS Template Injection**: Check if email/SMS templates can be manipulated by user input
   - **File Upload Abuse**: Check if file uploads can be used for phishing or malware distribution
   - **Phishing-Prone Features**: Check if application features can be exploited for phishing attacks

**Evidence Requirements**:
   - [Required] Code locations where user input is rendered or redirected
   - [Required] Security headers configuration (CSP, CORS, SameSite cookies)
   - [Required] HTTP request/response showing XSS, open redirect, or CSRF success

**Remediation**:
   - [High] Implement output encoding for all user-generated content (Difficulty: Easy)
   - [High] Validate redirect URLs against an allowlist (Difficulty: Easy)
   - [Medium] Implement CSRF protection for all state-changing endpoints (Difficulty: Easy)
   - [Medium] Serve uploaded files from a separate domain with safe headers (Difficulty: Medium)

**Pentest Validation**:
How to validate social engineering attack surface during penetration testing
  - Attack Steps: XSS testing: Inject <script>alert(1)</script> and <img src=x onerror=alert(1)> into all input fields
  ...
  - Tools: XSS detection with DOMPurify test payloads, curl -I "https://target/redirect?url=https://evil.com" to check redirect behavior, CSRF PoC page hosted externally to test cross-site request forgery, Burp Suite for intercepting and modifying requests
...
**Severity**: Critical | **CWE**: CWE-798, CWE-250, CWE-94, CWE-441, CWE-829, CWE-918 | **OWASP**: A05:2021 - Security Misconfiguration, A06:2021 - Vulnerable and Outdated Components, A01:2021 - Broken Access Control
...
**Check Flow**:
   - **Hardcoded Credential Detection**: Check if cloud credentials, API keys, or secrets are hardcoded in source code
   - **Container Privilege Escalation**: Check if containers run with excessive privileges enabling escape
   - **CI/CD Pipeline Injection**: Check if CI/CD pipelines process untrusted input without sanitization
   - **DNS Hijacking and Rebinding**: Check if DNS resolution is trusted without validation
   - **Dependency Supply Chain**: Check if dependencies have known vulnerabilities or supply chain risks
   - **Cloud Metadata Exposure**: Check if cloud instance metadata is accessible to the application

**Evidence Requirements**:
   - [Required] Code locations where credentials, container config, or CI/CD workflows are defined
   - [Required] Docker, Kubernetes, CI/CD, and cloud configuration files
   - [Required] HTTP request/response or network evidence of infrastructure access
   - [Required] Dependency versions and known vulnerabilities

**Remediation**:
   - [Critical] Remove all hardcoded credentials and use secrets management (Difficulty: Medium)
   - [High] Harden container security configuration (Difficulty: Easy)
   - [Critical] Sanitize untrusted input in CI/CD pipelines (Difficulty: Medium)
   - [High] Implement dependency scanning and lockfile enforcement (Difficulty: Easy)

**Pentest Validation**:
How to validate infrastructure attack vectors during penetration testing
  - Attack Steps: Credential scanning: Search source code for AWS keys, API tokens, and passwords using regex patterns
  ...
  - Tools: gitleaks or truffleHog for secret scanning, docker inspect <container> for privilege analysis, curl http://169.254.169.254/latest/meta-data/ (from within cloud instance), npm audit, yarn audit, or Snyk for dependency vulnerability scanning, Container Scanner (Trivy, Grype) for container image analysis
...
*Generated by HOS-Audit-Core | Version 0.3.0 (Dual Core) | 2026-06-17*
