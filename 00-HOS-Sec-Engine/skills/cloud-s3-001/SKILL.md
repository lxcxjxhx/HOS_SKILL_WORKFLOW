---
name: cloud-s3-001
description: "云存储桶（S3/OSS/COS）配置错误是最常见的云安全问题之一 适用于: 目标使用 AWS S3、阿里云 OSS、腾讯云 COS 等云存储服务作为静态资源托; Web 应用从云存储桶加载前端资源（JS/CSS/图片），URL 中包?bucket 名称; 应用允许用户上传文件到云存储桶，且未严格校验上传路径和文件类"
license: MIT
metadata:
...
`s3`, `oss`, `bucket`, `cloud storage`, `object storage`, `s3.amazonaws.com`, `aliyuncs.com`, `public read`, `public write`, `acl`, `bucket policy`, `cloudfront`, `oai`, `origin access identity`, `列举文件`, `桶策`, `公开读写`, `s3fs`, `aws s3 cp`, `ossutil`
...
3. 测试 bucket 读取权限：访?https://{bucket}.s3.{region}.amazonaws.com/
4. 测试 bucket 列举权限：访?https://{bucket}.s3.amazonaws.com/?list-type=2
5. 如果返回文件列表，分析文件类型和内容，寻找敏感数
6. 测试 bucket 写入权限：使?PUT 上传测试文件验证是否可写
7. 检?bucket ACL：aws s3api get-bucket-acl --bucket {bucket}
8. 检?bucket policy：aws s3api get-bucket-policy --bucket {bucket}
9. 检?CloudFront Distribution 是否回源到此 bucket，尝?OAI 绕过
10. 测试 CORS 配置：aws s3api get-bucket-cors --bucket {bucket}
11. 寻找敏感文件：配置文件（.env, config.json）、密钥文件、数据库备份、日志文
12. 如果 bucket 可写，考虑覆盖关键 JS 文件实现 XSS 或上传恶意文
13. 测试阿里?OSS：使?ossutil ls oss://{bucket}/ 检查访问权?
...
### S3 Bucket ACL ?Policy 检查与修改

检?bucket ?ACL ?Bucket Policy 配置，并在有权限时修改为公开访问
...
### 阿里?OSS 公开 Bucket 探测与利

针对阿里?OSS 公开 bucket 的探测、列举和下载操作
...
利用预签?URL 或签名算法漏洞绕过访问控

```
# 1. 预签?URL 包含访问密钥和签名，直接访问可绕?bucket 策略
