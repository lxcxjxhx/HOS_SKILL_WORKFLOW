/**
 * HOS-Sec-Engine V2 - MCP Security Audit Skill
 * 基于 SMCP (Secure Model Context Protocol) 论文的 MCP 协议安全检测技能
 * 覆盖 MCP 全工作流程的 23+ 类安全风险
 */
...
export const mcpSecuritySkills: AttackDefenseSkill[] = [
    {
        metadata: {
            ...
            name: 'MCP Protocol Security Audit',
            category: 'api',
            ...
            tags: [
                ...
                'tool-poisoning', 'prompt-injection', 'privilege-escalation',
                'session-management', 'supply-chain', 'audit',
        ...
        knowledge: {
            description: 'MCP (Model Context Protocol) 是 Anthropic 于 2024 年底推出的开放协议，用于统一 Agent 与外部工具的通信。随着 MCP 广泛采用，其安全性面临严峻挑战。SMCP (Secure MCP) 论文系统分析了 MCP 工作流程中的 23+ 类安全风险，涵盖 11 个交互阶段。本技能覆盖 MCP 连接认证、工具发现、任务分析、工具调用、结果返回和配置管理全流程的安全检测，帮助识别未授权访问、工具投毒、Prompt 注入、权限提升、供应链攻击等风险。',
            symptoms: [
        ...
        action: {
            checklist: [
                '阶段1: 连接认证检测 — 测试 MCP 服务端是否要求客户端身份认证',
                '阶段2: 会话管理检测 — 分析会话 ID 生成、过期和撤销机制',
                '阶段3: 工具发现检测 — 检查工具列表中的命名空间冲突/劫持',
                '阶段4: 工具元数据检测 — 分析工具描述和参数中的潜在投毒',
                '阶段5: Prompt 分析检测 — 测试 Prompt 模板的注入抗性',
                '阶段6: 工具选择检测 — 检查 Agent 工具选择的完整性约束',
                '阶段7: 调用转发检测 — 测试工具调用的输入验证和沙箱隔离',
                '阶段8: 执行环境检测 — 检查凭据存储、沙箱逃逸和权限控制',
                '阶段9: 结果返回检测 — 测试返回数据的路径遍历和内容投毒',
                '阶段10: 数据泄露检测 — 检查 Token 透传和跨租户数据隔离',
                '阶段11: 配置管理检测 — 审计版本管理、凭据轮换和配置基线',
                '综合评估: 跨阶段的多步攻击链（如工具链滥用）',
            ],
            techniques: [
            ...
            examples: [
                {
                    name: 'MCP 未认证访问检测',
                    description: '测试 MCP 服务端是否允许未认证客户端建立连接并获取工具列表',
                    content:
                        '测试步骤:\n' +
                ...
                {
                    name: 'MCP 工具名称冲突检测',
                    description: '检测是否存在同名或相似名称的 MCP 工具可能导致调用劫持',
                    content:
                        '检测步骤:\n' +
                ...
                {
                    name: 'MCP 间接 Prompt 注入检测',
                    description: '测试外部内容能否通过 MCP 的 resources/prompts 模块注入恶意指令',
                    content:
                        '检测步骤:\n' +
                ...
                {
                    name: 'MCP 工具链滥用检测',
                    description: '测试通过组合多个低风险 MCP 工具是否能实现高权限操作',
                    content:
                        '检测步骤:\n' +
                ...
                {
                    name: 'MCP 会话管理缺陷检测',
                    description: '测试 MCP 会话 ID 的生成强度、过期机制和撤销能力',
                    content:
                        '检测步骤:\n' +
