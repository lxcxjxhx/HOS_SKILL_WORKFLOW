...
export const idorDetectionSkills: AttackDefenseSkill[] = [
    {
        ...
        knowledge: {
            description: 'IDOR (Insecure Direct Object Reference)，OWASP API Security Top 10 中称为 BOLA (Broken Object Level Authorization)，是最常见的 API 安全漏洞之一。当 API 端点使用用户可控的标识符（数字 ID、UUID、用户名等）直接引用后端对象，且未在每次请求时验证当前用户是否有权访问该对象时，攻击者可通过修改标识符访问其他用户的资源。IDOR 危害极大，可导致数据泄露、数据篡改和权限提升，且自动化检测困难，需要深入理解业务逻辑',
            symptoms: [
            ...
            observations: [
                'IDOR 是 OWASP API Security Top 10 连续多年的 #1 漏洞，占比超过 30%',
                '数字 ID ?IDOR 最容易发现和利用，UUID/GUID 仅增加了发现难度而非阻止利用',
                'GraphQL IDOR 常被忽视，因为传统安全工具对 GraphQL 支持不足',
                '移动端 API 的 IDOR 更常见，因为开发者假设客户端不会修改请求参数',
                '批量操作 API（如 /api/users/bulk-delete）中的 IDOR 危害更大，可导致大规模数据泄露',
                '间接引用（如短链接、哈希 ID）如果映射表未做权限校验，同样存在IDOR',
                '很多系统对 GET 请求做了权限检查但遗漏了PUT/PATCH/DELETE',
            ...
            commonMistakes: [
                ...
                'GraphQL 查询中允许任意ID 过滤但未验证证访问权限',
                '批量 API 只验证第一个资源ID 的权限，忽略其余',
            ],
            notes: [
                'IDOR 难以自动化检测，需要结合业务逻辑理解什么是"越权"',
                'Burp Suite ?AuthMatrix ?Autorize 插件可辅助检查 IDOR',
                'OWASP API Security Top 10 2023 ?BOLA 列为 API1:2023',
                'GDPR、CCPA 等数据保护法规下，IDOR 导致的数据泄露可触发合规处罚',
                '测试 IDOR 需要至少两个不同权限级别的账户（A 和B），?A ?token 访问 B 的资源',
            ],
        },
        action: {
            checklist: [
                '注册两个不同用户账户（User A 的User B），获取各自的认证 token',
                '使用 User A 的token 访问资源，记录所有包含 ID 参数的请求',
                '将请求中的 ID 替换为 User B 的资源 ID，使用 User A 的token 发送请求',
                '检查响应是否返回User B 的数据（水平越权',
                '?PUT/PATCH/DELETE 请求重复上述测试（常被遗漏）',
                '尝试 ID 枚举：递增/递减 ID 值，观察是否返回不同用户数据',
                '测试间接引用：修改短链接、哈希 ID、base64 编码 ID',
                '检查 GraphQL 查询：传入其他用户的 ID，观察是否返回数据',
                '测试批量 API：在 ID 列表中加入不属于自己的资源 ID',
                '测试分页 API：通过遍历所有页获取不属于当前用户的资源',
                '检查响应中的信息泄露：其他用户 ID、内部路径、调试信',
                '验证多租户场景：尝试跨组织/租户访问资源',
            ],
            techniques: [
            ...
            examples: [
                {
                    name: '顺序 ID 枚举获取他人订单',
                    description: '通过递增订单 ID 遍历获取其他用户的订单详情',
                    content: "请求示例 (User A 查看自己的订单），:\n" +
                ...
                {
                    name: 'UUID/GUID 交叉访问',
                    description: '利用从公开接口获取的UUID 访问其他用户的资源',
                ...
                {
                    name: '请求体参数篡改(owner_id/account_id)',
                ...
                },
                {
                    ...
                    description: '在GraphQL 查询中替换对应ID 获取他人数据',
                    content: "正常查询 (User A 查看自己的信息），:\n" +
                        "POST /graphql\n" +
                        "Authorization: Bearer <UserA_Token>\n" +
                        'Content-Type: application/json\n\n' +
                        '{\n' +
                ...
                },
                {
                    ...
                    description: '在批量API 中添加他人资源ID 实现批量越权操作',
                    content: "正常请求 (删除自己的文档:\n" +
        ...
        validation: {
            indicators: [
                '使用 User A 的token 访问 User B 的资源 IDID 返回 200 和有效数据',
                '修改请求参数中的资源 ID 后成功获取或修改他人数据',
                '批量操作中成功处理了不属于自己的资源 ID',
                'GraphQL 查询返回其他用户的敏感信息',
            ...
            falsePositiveSigns: [
                '返回 403/401 说明权限检查正常生效',
                '返回 200 但数据是当前用户的（ID 不存在时回退到默认用户）',
                '返回空对象或 null（资源不存在而非权限通过',
                '返回 404（正确实现应返回 403 以避免 ID 枚举，但 404 也表明未越权',
                '响应中数据是公开的（如公开评论、公开资料），非真正越权',
            ],
        },
        defense: {
            recommendations: [
            ...
            mitigations: [
                '使用统一的授权中间件/装饰器，确保所有端点都经过权限检查',
                '实施 API 安全网关，集中处理认证和授权',
                '定期审查 API 端点的权限实现，尤其是新增端点',
                '使用自动化 IDOR 检测工具（如Burp Autorize、Postman 测试脚本',
