...
export const rateLimitBypassSkills: AttackDefenseSkill[] = [
    {
        ...
        knowledge: {
            description: '速率限制 (Rate Limiting) 绕过技术用于规避 API 对请求频率、数量或带宽的限制，使攻击者能够在受限条件下执行大量请求。速率限制通常基于 IP 地址、用户账户、API key 或会话实施。绕过技术包括 IP 轮换、请求头伪造、端点变异、时序攻击、并行请求和协议层绕过。成功绕过速率限制可导致暴力破解成功、数据大规模爬取、拒绝服务和 API 滥用。速率限制的实现质量差异很大，很多系统存在可被利用的盲区别',
            symptoms: [
            ...
            notes: [
                '速率限制绕过是暴力破解、数据爬取和 API 滥用的前提技术',
                'OWASP API Security Top 10 将速率限制缺失列为 API4:2023',
                '现代速率限制实现推荐使用滑动窗口 + 令牌桶+ 行为分析的组合',
                'Cloudflare、AWS WAF 等提供托管速率限制服务，但配置不当仍可绕过',
                '分布式系统中的速率限制需要全局计数器（如 Redis），本地计数器无效',
            ],
        },
        action: {
            checklist: [
                ...
                '尝试 IPv6 地址轮换或代理池轮换',
                '测试并行请求（Race Condition）是否在计数器更新前完成本',
                '测试 GraphQL batching/nesting 是否绕过速率限制',
                '测试时间窗口边界（滑动窗口 vs 固定窗口',
                '检查速率限制响应头，分析限制规则',
                '尝试 API key 轮换或枚举未使用的 API key',
            ],
            techniques: [
            ...
            examples: [
                {
                    name: 'X-Forwarded-For 头伪造绕过 IP 速率限制',
                    description: '通过伪造 X-Forwarded-For 请求头欺骗服务端使用不同IP 进行速率限制计数',
                    content: `正常请求 (触发速率限制):
...
原理: 速率限制通常按 HTTP 方法分别配置
     ...
     某些后端框架对非标准方法的处理可能导致绕过`,
                ...
                {
                    name: '端点枚举与变体绕过',
                    description: '同一功能可能有多个API 端点，速率限制可能只应用于部分端点',
...
适用: 非原子速率限制计数器、无分布式锁的实现`,
                },
                {
                    ...
                    description: '在单个 GraphQL 请求中包含多个查询/变更，绕过基于请求数的速率限制',
                    content: `场景: GraphQL API 限制每分钟 10 个请求
正常请求:
POST /graphql
...
防御: 按查询复杂度（深度、字段数、解析成本）计数而非请求数`,
                },
                {
                    ...
                    description: '利用令牌桶或固定窗口算法的时间特性，在特定时间点集中发送请求',
                    content: `场景: 令牌桶算法，每秒添加 10 个令牌，桶容量 10
...
原理: 固定窗口在边界处允许 2 倍于限制流量的突发，     利用这个时序特性可在短时间内发送超额请求`,
                ...
                {
                    name: 'API Key 枚举与轮',
                    description: '通过枚举或轮?API key 分散速率限制，每?key 使用其独立配置',
...
防御: API key 绑定到账户，全局速率限制，而非?key 独立限制`,
                ...
                {
                    name: 'User-Agent 与指纹轮换',
                    description: '通过轮换 User-Agent 和其他指纹信息绕过基于客户端标识的速率限制',
                    content: `场景: 速率限制基于 User-Agent 或设备指纹），
User-Agent 轮换列表:
...
原理: 一些速率限制系统使用多维度指纹（UA + Cookie + TLS），     如果其中一个维度可伪造，可绕过基于该维度的限制`,
        ...
        validation: {
            indicators: [
                '修改 X-Forwarded-For 后 429 响应变为 200/401',
                '切换 HTTP 方法后速率限制不再触发',
                '访问端点变体时速率限制计数器独',
                '并行请求中多个请求在限制触发前成功执行',
                'GraphQL batching 请求返回大量数据但未触发速率限制',
                '使用不同 API key 时速率限制计数器重置',
                '时间窗口边界处可发送超额请求',
            ],
            successSigns: [
                '429 响应消失，请求正常处理',
        ...
        defense: {
            ...
            mitigations: [
                ...
                '定期审查和更新速率限制策略，覆盖新增端点',
                '?API key 实施轮换和生命周期管理，及时撤销泄露 key',
                '使用 IP 信誉服务识别和阻止已知恶意 IP',
