...
export const jwtAttackSkills: AttackDefenseSkill[] = [
    {
        ...
        knowledge: {
            description: 'JWT (JSON Web Token) 攻击技术利?JWT 实现中的设计缺陷、配置错误和协议层漏洞，通过修改算法、伪造签名、操纵声明等方式绕过身份验证。JWT ?Header.Payload.Signature 三部分组成，其安全性完全依赖于签名密钥的保密性和算法的正确实现。实战中发现大量系统存在算法混淆漏洞、弱密钥、kid 注入等问题，攻击者可在不知道密钥的情况下构造有?token',
            symptoms: [
            ...
            observations: [
                '超过 60% ?JWT 实现存在算法混淆漏洞，尤其是 Java (jjwt) ?Python (PyJWT < 2.0) 生',
                'HS256/RS256 混淆?.NET (JwtSecurityTokenHandler) ?Node.js (jsonwebtoken) 中依然常',
                'kid 参数路径遍历 (../../../etc/passwd) ?Go ?Java 实现中广泛存',
                '部分系统?token 过期后仍有一?grace period（通常 5-30 分钟），可被利用',
                'JWT ?payload ?Base64 编码而非加密，敏感数据（用户 ID、角色、邮箱）完全暴露',
            ...
            commonMistakes: [
                ...
                '使用 "none" 算法时未在服务端显式禁用，依赖库的默认行',
                'HS256 密钥使用 "secret"、"password123" 等弱密钥',
                '未验?aud (audience) ?iss (issuer)，token 可在不同服务间重',
                'kid 参数直接拼接文件路径，未做路径规范化或白名单校验',
                'jku/x5u 允许任意 URL，未限制域名或使?TLS 验证',
                '修改 alg ?HS256 但使?RS256 公钥?PEM 格式而非原始公钥字节',
                '只关注签名验证而忽略了 token 过期、重放、撤销等生命周期管?',
            ],
            notes: [
                ...
                'jwt-cracker/hashcat 模式 16500 支持 HS256 密钥爆破，速度可达数十亿次/',
                'kid 注入可能升级?RCE（命令注入）?LFI（本地文件包含），危害远?JWT 绕过本身',
                'JWS (RFC 7515) ?JWK (RFC 7517) 规范定义了完整的密钥管理框架，但很多实现未遵?',
            ],
        },
        action: {
            checklist: [
                '提取目标 JWT，使?jwt.io 或命令行工具解码 header ?payload',
                '检?header 中的 alg 字段，确认签名算法类',
                '尝试?alg 改为 "none"，移?signature，观察服务端响应',
                '如果 alg ?RS256，尝试获取公钥，?alg 改为 HS256，用公钥作为 HMAC 密钥重新签名',
                '使用 jwt-cracker、hashcat (mode 16500) ?john-the-ripper 爆破 HS256 secret',
                '检?kid 参数是否存在，尝试路径遍?(../../../etc/passwd) ?SQL 注入',
                '修改 payload 中的敏感声明（role、isAdmin、sub、exp），重新签名测试',
                '检?jku/x5u 参数是否允许外部 URL，尝试控制密钥来',
                '测试过期 token 是否仍然有效，测试修?exp 为未来时',
                '验证 iss ?aud 声明，测?token 跨服务复',
                '检?token 是否绑定 IP/User-Agent，测试重放攻?',
            ],
            techniques: [
            ...
            examples: [
                {
                    name: 'none 算法攻击',
                    description: '?JWT header 中的 alg 设置?"none"，移除签名部分，服务端可能接受无签名 token',
                    content: `原始 JWT:
...
原理: JWT 规范支持 "none" 算法用于调试，部分服务端库默认接?alg=none 且不对签名进行验?适用: 适用于使用旧?JWT 库（PyJWT < 1.5, jsonwebtoken < 9）或未显式禁?none 算法的系统`,
                },
                {
                    ...
                    content: `步骤 1: 获取目标服务器的 RSA 公钥（通常?/.well-known/jwks.json 或证书中提取?
步骤 2: 修改 JWT Header:
{"alg":"HS256","typ":"JWT"}
...
适用: 适用?jwt-cpp、PyJWT < 2.0、jsonwebtoken 旧版本等未区分对?非对称验证路径的库`,
                ...
                {
                    name: 'HS256 弱密钥暴力破',
                    description: '使用字典攻击或规则攻击爆?JWT HMAC 签名密钥',
                    content: `目标 JWT:
eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwicm9sZSI6InVzZXIiLCJpYXQiOjE1MTYyMzkwMjJ9.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c
...
适用: 适用于获取密钥后的权限提升场景，或对签名验证不严格的系统`,
                },
                {
                    ...
                    description: '利用 jku (JWK Set URL) ?x5u (X.509 URL) 头部参数，指定攻击者控制的密钥 URL',
                    content: `原始 JWT Header:
...
适用: 适用于支?jku/x5u 动态密钥加载且未限?URL 白名单的系统`,
                ...
                {
                    name: '过期 Token 重用?exp 篡改',
                    description: '利用 token 过期验证不严或修?exp 声明为未来时',
...
原理: 部分 JWT 库或自定义验证逻辑忽略 exp 字段
     或使用不安全的默认行为（如验证失败时返回默认用户）`,
        ...
        validation: {
            indicators: [
                '使用 alg=none 构造的无签?token 通过服务端验',
                'RS256→HS256 混淆后，用公钥签名的 token 通过验证',
                '弱密钥爆破成功，可用已知密钥伪造任?token',
            ...
            falsePositiveSigns: [
                '服务端返?200 但实际是缓存响应而非真实鉴权通过',
                'alg=none 被拒绝返?401，说明系统已禁用 none 算法',
                'HS256/RS256 混淆返回 401，说明服务端正确区分了对?非对称验',
                '弱密钥爆破超时未发现密钥，密钥强度足',
                '过期 token 返回 401，说?exp 验证正确实现',
                '修改声明后返?403，说明服务端有额外权限校?',
            ],
        },
        defense: {
            recommendations: [
            ...
            mitigations: [
                '升级到最?JWT 库版本（PyJWT 2.0+, jsonwebtoken 9+, jjwt 0.12+',
                '?API Gateway 层统一 JWT 验证，避免各微服务实现不一',
                '使用 JWKS (JSON Web Key Set) 端点管理公钥，定期轮换密',
                '实施速率限制防止密钥爆破（针对登录和 token 刷新接口',
                '监控异常 JWT 验证失败模式，及时发现攻击行?',
            ],
            references: [
