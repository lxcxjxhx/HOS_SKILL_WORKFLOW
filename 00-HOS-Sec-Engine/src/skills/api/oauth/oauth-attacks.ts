...
export const oauthAttackSkills: AttackDefenseSkill[] = [
    {
        ...
        knowledge: {
            description: 'OAuth 2.0 和OpenID Connect (OIDC) 是现代应用最广泛使用的授权和认证协议。OAuth 流程攻击利用协议实现中的配置错误、参数校验不严和流程设计缺陷，通过拦截授权码、操纵重定向 URI、绕过 CSRF 保护、窃取 token 等方式获取未授权访问。OAuth 的安全性依赖于多个安全机制的正确实现：state 参数防 CSRFCSRF、redirect_uri 严格匹配、PKCE 防授权码拦截、token 安全传输和存储。任何一个环节的疏漏都可能导致认证绕过或 token 泄露',
            symptoms: [
                ...
                'PKCE 未正确实现或 code_verifier 未验证证，授权码可被拦截使用',
        ...
        action: {
            checklist: [
                ...
                '检查 redirect_uri 参数是否接受任意 URL 或仅白名单 URL',
                '验证 state 参数是否存在、是否随机、是否在回调时验证',
                '检查 PKCE 是否实施（code_challenge ?code_verifier',
                '测试授权码是否可被截获并兑换为access_token',
                '检查 token 传输方式（HTTPS、是否在 URL fragment 中暴露）',
                '测试 Referer 头是否会泄露的 token',
                '检查 id_token 签名验证和aud 声明验证',
                '测试 scope 验证是否严格',
                '检查 OAuth 客户端密钥是否暴露在客户端代码中',
                '测试动态客户端注册是否存在未审批注册漏',
                '检查 token 刷新流程是否安全（refresh token 轮换机制',
            ],
            techniques: [
            ...
            examples: [
                {
                    ...
                    description: '截获 OAuth 授权码并在攻击者控制的客户端兑换 access_token',
                ...
                },
                {
                    ...
                    description: '修改 redirect_uri 参数将授权码和 token 发送到攻击者控制的地址',
                ...
                },
                {
                    ...
                    description: 'OAuth 流程中缺少 state 参数验证，攻击者可强制用户使用攻击者账户登',
                ...
                },
                {
                    ...
                    content: "场景: SPA 使用 Implicit Grant，access_token 和URL fragment 中\n\n" +
                ...
                },
                {
                    ...
                    description: '利用 Implicit Grant 流程中 token 从 URL fragment 暴露的弱点窃取 token',
                ...
                {
                    name: 'Scope 提升攻击',
                    description: '请求超出预期的 OAuth scope 获取更高权限制access_token',
                    content: "正常授权请求:\n" +
                        "https://oauth-provider.com/authorize?\n" +
                        "  response_type=code\n" +
                        "  &client_id=CLIENT_ID\n" +
