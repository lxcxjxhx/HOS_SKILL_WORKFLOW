...
export const windowsPrivEscSkills: AttackDefenseSkill[] = [
    {
        ...
        knowledge: {
            description: 'Windows 权限提升技术用于在获取低权限 shell 后提升到 SYSTEM 或 Administrator 权限。Windows 系统存在多种提权路径：服务配置错误、可写系统路径、令牌操作、UAC 绕过、内核漏洞利用等。提权成功取决于系统配置、权限和补丁级别',
            symptoms: [
