...
export const s3MisconfigSkills: AttackDefenseSkill[] = [
    {
        ...
        knowledge: {
            ...
            observations: [
                'S3 bucket 名称是全局唯一的，可通过暴力枚举 DNS 记录（{name}.s3.amazonaws.com）发现目标 bucket',
                '阿里云 OSS ?bucket 名称在区域内唯一，可通过 ossutil ls 配合字典枚举',
                '很多企业的 bucket 名称遵循命名模式，如 {company}-assets、{company}-prod-data、{company}-backup',
                'S3 bucket 如果配置了 Website Hosting，可以通过 Website 端点绕过部分 ACL 限制',
                '覆盖前端 JS 文件（如 main.js、app.js）可以实现持久的 XSS 攻击，影响所有访问',
                '阿里云 OSS 支持 STS 临时凭证，如果泄露可直接操作 bucket',
                '某些 bucket 设置了 IP 白名单，但白名单仅针对 bucket policy，ACL 可能仍然公开',
                'CloudFront 自定义域名可能指向 S3 bucket，通过 CNAME 记录可反向查找 bucket 名称',
                'S3 Object Lambda ?S3 Access Points 提供了额外的访问控制层，但配置复杂容易出错',
            ],
            commonMistakes: [
                '只测试了 S3 REST API 端点，忽略了 Website 端点（{bucket}.s3-website-{region}.amazonaws.com',
                '未测试不同区域的 S3 端点（bucket 可能在非默认区域',
                '遇到 403 就认为 bucket 安全，但 403 可能仅表示 ListBucket 被拒绝，GetObject 可能仍然允许',
                '未尝试上传小文件测试可写性，仅测试了读取权限',
                '忽略了 bucket 策略中的 Condition 字段可能被绕过的情况（如 aws:SourceIp 可通过内网 SSRF 绕过',
                '未检查 CloudFront Distribution 的 OriginOrigin 配置，可能暴露私有 bucket 的访问方式',
                '只测试了匿名访问，未测试使用已知凭证（如泄露的 AccessKey）后的权限提升',
            ],
            notes: [
                '发现 bucket 后，优先枚举文件列表寻找敏感信息（配置文件、密钥、备份文件、数据库 dump',
                '可写 bucket 的攻击价值远大于只读 bucket — 可以覆盖关键文件实现供应链攻',
                '部分 bucket 设置了 "Requester Pays"，访问需要额外 header，不影响利用但需要知道',
                'S3 bucket 版本控制（Versioning）开启后，删除文件实际是创建删除标记，历史版本仍可访',
                '利用 bucket 进行钓鱼攻击：上传伪造的登录页面，利用bucket 域名和HTTPS 增加可信度',
                '监控 bucket 的最佳方式是开启 S3 Access Logging 和CloudTrail 数据事件',
            ],
        },
        action: {
            checklist: [
                ...
                '测试 bucket 读取权限：访问 https://{bucket}.s3.{region}.amazonaws.com/',
                '测试 bucket 列举权限：访问 https://{bucket}.s3.amazonaws.com/?list-type=2',
                '如果返回文件列表，分析文件类型和内容，寻找敏感数据',
                '测试 bucket 写入权限：使用 PUT 上传测试文件验证是否可写',
                '检查 bucket ACL：aws s3api get-bucket-acl --bucket {bucket}',
                '检查 bucket policy：aws s3api get-bucket-policy --bucket {bucket}',
                '检查 CloudFront Distribution 是否回源到此 bucket，尝试 OAI 绕过',
                '测试 CORS 配置：aws s3api get-bucket-cors --bucket {bucket}',
                '寻找敏感文件：配置文件（.env, config.json）、密钥文件、数据库备份、日志文件',
                '如果 bucket 可写，考虑覆盖关键 JS 文件实现 XSS 或上传恶意文件',
                '测试阿里云 OSS：使用 ossutil ls oss://{bucket}/ 检查访问权限',
            ],
            techniques: [
            ...
            examples: [
                ...
                {
                    name: 'S3 Bucket ACL ?Policy 检查和修改与修改',
                    description: '检查 bucket 的 ACL 和ACL ?Bucket Policy 配置，并在有权限时修改为公开访问',
                ...
                {
                    name: '阿里云OSS 公开 Bucket 探测与利用',
                    description: '针对阿里云OSS 公开 bucket 的探测、列举和下载操作',
                ...
                },
                {
                    ...
                    description: '利用预签名URL 或签名算法漏洞绕过访问控制',
                    content: '# 1. 预签名 URL 包含访问密钥和签名，直接访问可绕过 bucket 策略\n' +
        ...
        validation: {
            indicators: [
                '访问 bucket 根路径返回 XML 格式的ListBucketResult ?ListBucketResult 内容',
                'PUT 请求返回 HTTP 200 且无错误，确认 bucket 可写',
                '下载的文件包含敏感信息（配置文件、密钥、数据库凭证、用户数据）',
                'bucket ACL ?Grant 包含 AllUsers 或AuthenticatedUsers',
                'bucket policy ?Policy 中的 Principal"*" ?Effect ?Allow',
            ...
            falsePositiveSigns: [
                'bucket 返回 404（NoSuchBucket）表示 bucket 不存在而非配置错误',
                '403 AccessDenied 且错误信息明确说明权限不足（非配置错误）',
                'bucket 配置了 Requester Pays，需要额外 header 才能访问（非公开访问',
                '响应为 XML 但为空列表（bucket 存在但确实没有文件）',
                'CloudFront 返回 403 ?S3 端点也返回403（OAI 配置正确），',
            ],
        },
        defense: {
            recommendations: [
            ...
            mitigations: [
                ...
                '审查 CloudTrail 日志，确认是否有未授权的 bucket 访问或数据泄露',
                '轮换所有可能泄露的 AccessKey ?SecretKey',
                '对可写bucket 中上传的文件进行完整性校验和恶意代码扫描',
                '如果 bucket 曾被用于 XSS 攻击，清除所有可疑文件并通知受影响用户',
