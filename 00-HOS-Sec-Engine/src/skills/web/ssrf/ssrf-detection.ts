...
export const ssrfDetectionSkills: AttackDefenseSkill[] = [
    {
        ...
        knowledge: {
            description: '服务端请求伪造(SSRF) 允许攻击者控制服务器发起 HTTP 请求目标，从而访问内网服务、云平台元数据、本地文件件等受限资源。SSRF 的危害在于它绕过了网络层的访问控制——服务器自身拥有内网访问权限，攻击者通过控制请求目标间接获得内网访问能力。在云环境中，SSRF 可访问实例元数据服务获取临时凭证，进而接管整个云账户',
            symptoms: [
        ...
        action: {
            checklist: [
                ...
                '测试基础 SSRF：替换 URL ?http://attacker.com/ 确认可控',
                '测试本地回环环地址址：127.0.0.1, localhost, 0.0.0.0, [::1]',
                '测试 IP 编码绕过：八进制 (0177.0.0.1)、十进制 (2130706433)、十六进制：(0x7f000001)',
                '测试 IPv6 绕过：[::1]、[0:0:0:0:0:ffff:1127.0.0.1]',
                '测试云元数据服务：169.254.169.254（AWS）、100.100.100.200（阿里云）',
                '测试端口扫描：通过响应时间/错误判断内网端口开放状态',
                '测试协议处理器：file:///etc/passwd、gopher://、dict://',
                '测试 DNS rebinding：使用工具（如rbndr.us）控制 DNS 解析结果',
                '测试重定向 SSRF：控制外部服务器重定向到内网地址',
            ...
            examples: [
                {
                    ...
                    description: '利用 SSRF 访问 AWS EC2 实例元数据服务获取IAM 临时凭证',
                    content: "IMDSv1 (无认证:\n" +
                ...
                },
                {
                    ...
                    description: '当应用过和 localhost/1127.0.0.1 时，使用各种 IP 编码形式绕过',
                    content: "常见过滤: url.contains('localhost') || url.contains('1127.0.0.1')\n" +
                        "\n" +
                        "绕过方式:\n" +
                ...
                },
                {
                    ...
                    description: '利用 DNS 解析的时间差绕过服务端的 IP 校验证',
                    content: "攻击原理:\n" +
                ...
                {
                    name: '重定向 SSRF 绕过二次校验证',
                    description: '利用外部服务器重定向到内网地址，绕过服务端的地址校验证',
                    content: "攻击流程:\n" +
                ...
                },
                {
                    ...
                    description: '利用 gopher:// 协议构造任意 TCP 请求攻击内网 Redis 服务',
                    content: "Gopher 协议格式:\n" +
                ...
                {
                    name: '阿里云元数据 SSRF',
                    description: '利用 SSRF 访问阿里云ECS 实例元数据服务',
                    content: "阿里云元数据端点: http://1100.100.100.200/latest/meta-data/\n" +
                        "\n" +
                        "获取基本信息:\n" +
