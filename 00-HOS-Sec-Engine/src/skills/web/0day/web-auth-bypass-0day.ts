...
export const authBypass0daySkills: AttackDefenseSkill[] = [
  {
    ...
    action: {
      checklist: [
        '绘制目标认证状态机图，识别所有认证路径（密码登录、OAuth、MFA、SSO、Magic Link）',
        '对每条认证路径测试参数缺失、参数篡改、并发请求等异常场景',
        'OAuth：测试 redirect_uri 精确匹配、state 随机性、PKCE 实施、授权码绑定',
        'JWT：测试 none 算法、RS256→HS256 混淆、kid 注入、jku/x5u 控制、claim 篡改',
        'MFA：测试验证码可预测、时序竞争、会话固定、MFA 状态与认证状态解耦',
        'SAML/OIDC：测试断言重放、Audience 校验、签名算法降级、XML 注释注入',
        'Magic Link/Passwordless：测试 token 熵、有效期、重放、跨用户复用',
        '密码重置：测试 token 与用户身份绑定、token 失效机制、Referer 泄露',
        '会话管理：测试会话固定、session ID 更换、跨域 session 泄露',
        '使用 Burp Suite 对认证请求进行自动化模糊测试和时序分析',
      ],
      techniques: [
      ...
      examples: [
        {
          name: 'OAuth redirect_uri 子域名绕过',
          description: '利用 OAuth 服务提供商对 redirect_uri 的子域名通配校验缺陷，将授权码发送到攻击者控制的子域名',
          content:
          ...
          applicableScenarios: ['OAuth 第三方登录', '社交登录集成', '企业 SSO'],
        },
        {
          name: 'JWT none 算法认证绕过',
          description: '将 JWT header 中的 alg 修改为 "none" 并移除签名，利用服务端未显式禁用 none 算法接受无签名 token',
          content:
          ...
          applicableScenarios: ['JWT 认证 API', '微服务间 JWT 认证', 'API Gateway JWT 验证'],
        },
        {
          name: 'MFA 时序竞争绕过',
          description: '利用 MFA 验证状态与会话状态更新的竞态条件，在 MFA 验证完成前发起认证完成请求',
          content:
          ...
          applicableScenarios: ['MFA/2FA 登录流程', '企业身份认证', '银行/金融应用'],
        },
      ],
    },
    validation: {
