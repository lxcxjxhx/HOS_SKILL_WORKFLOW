/**
 * HOS-Sec-Engine V2 - Web WAF Bypass 0day Skills
 * WAF 绕过 0day 专项 Skill
 */

import { AttackDefenseSkill, DEFAULT_SKILL_RUNTIME } from '../../../types/skill';

export const wafBypass0daySkills: AttackDefenseSkill[] = [
  {
    metadata: {
    ...
    action: {
      checklist: [
        '识别 WAF 类型（Cloudflare、ModSecurity、AWS WAF、阿里云 WAF、Imperva 等）',
        '分析被拦截的 payload，精确定位被检测的关键字或模式',
        '尝试 URL 编码（单次、双重、三重）、Unicode 编码、HTML 实体编码',
        '尝试关键字分割：使用注释（/**/、--、#）、空白符（%09、%0a、+）、换行符',
        '尝试大小写混合：SeLeCt、UnIoN、ScRiPt',
        '尝试协议层绕过：HTTP/2、分块传输编码、multipart 边界操纵',
        '尝试 HTTP 头注入：将 payload 放在 X-Forwarded-For、Referer、User-Agent 等头中',
        '尝试请求走私：利用 Content-Length 与 Transfer-Encoding 的解析差异',
        '尝试 WAF 不检查的位置：Cookie 值、JSON 字段、XML 属性、GraphQL 查询',
        '验证绕过后的 payload 是否真正到达后端（使用 DNS 外带或时间延迟确认）',
      ],
      techniques: [
      ...
      examples: [
        {
          name: 'SQL 注入 WAF 绕过 (Cloudflare)',
          description: '使用多种编码和混淆技术绕过 Cloudflare WAF 的 SQL 注入检测',
          content:
          ...
          applicableScenarios: ['Cloudflare 保护的网站', 'SQL 注入点被 WAF 拦截', 'API 端点 SQL 注入'],
        },
        {
          name: 'HTTP 请求走私绕过 WAF',
          description: '利用前端 WAF 与后端服务器对 HTTP 请求的解析差异，绕过 WAF 检测',
          content:
          ...
          applicableScenarios: ['反向代理后的 WAF', 'CDN + 源站架构', '多层代理环境'],
        },
        {
          name: 'XSS WAF 绕过技术',
          description: '使用 HTML 解析差异、事件处理器变体、CSP 策略缺陷绕过 WAF 的 XSS 过滤',
          content:
          ...
          applicableScenarios: ['XSS 过滤 WAF', 'CSP 策略绕过', '富文本编辑器 XSS'],
        },
      ],
    },
    validation: {
