...
export const commandInjectionSkills: AttackDefenseSkill[] = [
    {
        ...
        knowledge: {
            description: '命令注入允许攻击者通过操控应用程序传递给操作系统 shell 的参数执行任意系统命令。与代码注入不同，命令注入直接执行操作系统命令，通常具有与 Web 应用进程相同的权限级别。命令注入可分为有回显（输出直接返回）和盲注（无输出回显，需通过时间延迟、DNS 查询、HTTP 请求等带外通道验证）。现代应用中命令注入较少见但仍存在于网络诊断工具、图像处理、文档转换、系统管理等场景',
            symptoms: [
                '用户输入直接拼接到系统命令字符串',
        ...
        action: {
            ...
            examples: [
                {
                    name: '基础 OS 命令注入',
                    description: '利用命令分隔符在合法命令后注入额外命令',
                ...
                {
                    name: '关键字黑名单绕过',
                    description: '当命令关键字（cat、ls、whoami 等）被过滤时，使用变体绕过',
                ...
                {
                    name: 'Base64 编码命令注入',
                    description: '将命令进行 base64 编码后通过解码执行，绕过关键字和特殊字符过滤',
                    content: "编码命令:\n" +
                ...
                },
                {
                    ...
                    description: '针对 Java Runtime.exec() 的特殊命令注入技术',
