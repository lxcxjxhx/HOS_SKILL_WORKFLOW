...
export const xssFilterBypassSkills: AttackDefenseSkill[] = [
  {
    ...
    trigger: {
      ...
      indicators: [
        'payload被转义但未消除攻击面',
        'CSP header 存在但配置宽松',
        'WAF拦截<script>但放行<img>/<svg>',
        '输入在HTML属性/JS/URL上下文中反射',
      ],
    },
    knowledge: {
      description:
      ...
      observations: [
        '现代 WAF 对 <script> 和 onerror/onload 检测严格，但对 ontoggle、onanimationend 等冷门事件覆盖不足',
        'CSP 中 script-src \'unsafe-inline\' 等价于无 CSP 防护',
        'Angular 中 [innerHTML] 和 bypassSecurityTrustHtml 常被开发者误用',
        'React 的 dangerouslySetInnerHTML 和 AngularJS 的 ng-bind-html 是常见注入点',
        'SVG 标签在多数 WAF 中被放行，但 <svg><script> 或 SVG 事件处理器可执行 JS',
        'DOMPurify 等净化库在旧版本中存在绕过漏洞，需保持更新',
        'URL 编码、HTML 实体编码、JS Unicode 编码在特定上下文中可被浏览器自动解码',
      ],
      commonMistakes: [
        '仅测试 <script> 标签，忽略事件处理器和 SVG/MathML 标签',
        '未识别 XSS 上下文（HTML body、属性值、JS 字符串、URL），导致 payload 无效',
        'CSP 绕过时未分析全部 CSP 策略（多个策略取交集）',
        'DOM XSS 测试时只关注 URL 参数，忽略 postMessage、localStorage 等 source',
        '未考虑浏览器自动解码行为，手动双重编码导致 payload 失效',
      ],
      notes: [
        'XSS 上下文识别是关键：同一 payload 在 HTML body 有效但在属性值中无效',
        'CSP 绕过需要完整分析所有 CSP 指令，特别是 script-src、object-src、base-uri',
        'DOM XSS 测试应使用工具（如 DOM Invader）辅助分析数据流',
        '现代前端框架自带 XSS 防护，但开发者显式绕过时（如 v-html）风险极高',
        'WAF XSS 规则更新频繁，但新事件处理器和 HTML5 特性常存在检测盲区',
      ],
    },
    action: {
      checklist: [
        '识别 XSS 类型（反射型、存储型、DOM 型）和注入上下文',
        '分析 CSP 策略（script-src、object-src、base-uri、default-src 等）',
        '确定输入被编码/过滤的方式（HTML 实体、URL 编码、关键字删除 等）',
        '测试 SVG/MathML 标签和事件处理器绕过',
        '测试 DOM XSS source（location、URL、postMessage、localStorage 等）',
        '尝试编码绕过（HTML 实体、Unicode、Hex、JS 八进制）',
        '针对 CSP 绕过测试：nonce 复用、base-uri 劫持、JSONP 端点',
        '前端框架特定测试：Angular sandbox、React dangerouslySetInnerHTML',
        '验证 payload 在目标浏览器中是否实际执行',
      ],
      techniques: [
      ...
      examples: [
        {
          name: 'SVG onload 事件绕过 WAF',
          description: '利用 SVG 标签和 onload 事件执行 JS，绕过对 <script> 标签的检测',
          content:
        ...
        {
          name: 'IMG 标签事件处理器绕过',
          description: '利用 <img> 标签的 onerror 等事件处理器执行 XSS',
          content:
            "原始 payload: <script>alert(1)</script>\n" +
        ...
        {
          name: 'CSP unsafe-inline 策略绕过',
          description: '当 CSP 包含 script-src \'unsafe-inline\' 时，直接执行内联脚本',
          content:
        ...
        {
          name: 'CSP base-uri 劫持绕过',
          description: '当 CSP 未限制 base-uri 时，通过 <base> 标签劫持相对路径脚本加载',
          content:
        ...
        {
          name: 'Angular 模板注入 XSS',
          description: '利用 Angular 模板表达式绕过 CSP 和执行 XSS',
          content:
        ...
        {
          name: 'DOM XSS 通过 postMessage 数据流',
          description: '利用 postMessage 传递恶意数据到 DOM sink 实现 XSS',
          content:
        ...
        {
          name: 'WAF 关键字删除绕过',
          description: 'WAF 删除 script 关键字后，嵌套拼接形成有效 payload',
          content:
        ...
        {
          name: 'React dangerouslySetInnerHTML 绕过',
          description: '利用 React 的 dangerouslySetInnerHTML 执行 XSS',
          content:
    ...
    validation: {
      indicators: [
        '弹窗或 JS 代码在目标页面中执行',
        'fetch/beacon 请求到达攻击者服务器',
        'document.cookie 被成功外带',
        'DOM 中出现预期的恶意元素',
        'CSP 违规报告未触发（说明绕过成功）',
      ],
      successSigns: [
      ...
      falsePositiveSigns: [
        '弹窗由页面自身 JS 触发而非注入的 payload',
        'CSP 违规报告仍被发送（绕过不完整）',
        'payload 在测试浏览器有效但在目标用户浏览器无效',
        '仅在开发者工具控制台执行，未在页面上下文执行',
      ],
    },
    defense: {
      recommendations: [
      ...
      mitigations: [
        '启用 HttpOnly 标志减少 XSS 对 cookie 的窃取影响',
        '使用 SameSite cookie 属性防止 CSRF 结合 XSS',
        '部署 Subresource Integrity (SRI) 防止 CDN 资源篡改',
        '定期使用 CSP Evaluator 工具评估 CSP 策略有效性',
        '在 CI/CD 中集成自动化 XSS 扫描',
      ],
      references: [
