...
export const xxeInjectionSkills: AttackDefenseSkill[] = [
    {
        ...
        knowledge: {
            description: 'XML 外部实体 (XXE) 注入利用 XML 解析器对 DTD（文档类型定义）中外部实体的处理机制，读取服务器本地文件、发起服务端请求、执行内网端口扫描等。当 XML 解析器配置为允许处理外部实体时，攻击者通过构造恶?DTD 引入外部资源，将其内容嵌?XML 文档返回或外带至攻击者服务器。XXE 在现代应用中仍常见于 SOAP API、旧系统、文件解析器（Office、PDF、SVG）等场景',
            symptoms: [
        ...
        action: {
            ...
            examples: [
                ...
                {
                    name: 'OOB XXE 带外数据外带',
                    description: '?Blind XXE 场景下，通过外部 DTD 将文件内容发送到攻击者服务器',
                    content: "攻击流程:\n" +
                ...
                {
                    name: '参数实体注入复杂攻击',
                    description: '利用参数实体 (%entity;) ?DTD 内部构造多阶段攻击',
                    content: "参数实体只能?DTD 内部使用，但可构造复杂的攻击?\n" +
                        "\n" +
                        "Payload:\n" +
                        "<?xml version=\"1.0\"?>\n" +
                        "<!DOCTYPE root [\n" +
                ...
                {
                    name: 'XInclude 攻击',
                    description: '?DOCTYPE 被过滤时，使?XInclude 机制引入外部文件',
                    content: "XInclude ?XML 标准的一部分，用于包含外?XML 文档:\n" +
                        "\n" +
                        "Payload:\n" +
                        "<?xml version=\"1.0\"?>\n" +
                ...
                {
                    name: 'SVG 文件上传 XXE',
                    description: '通过上传包含 XXE payload ?SVG 文件利用文件上传入口',
                    content: "SVG 本质?XML 格式，可直接嵌入 XXE payload:\n" +
                        "\n" +
                        "evil.svg:\n" +
                ...
                },
                {
                    ...
                    content: "前提: PHP 安装?expect 扩展（非默认安装）\n" +
                        "\n" +
                        "Payload:\n" +
                        "<?xml version=\"1.0\"?>\n" +
                        "<!DOCTYPE foo [\n" +
                ...
                {
                    name: 'Java XXE 多协议利',
                    description: '利用 Java XML 解析器的多种协议实现文件读取?SSRF',
                    content: "Java 支持的协?\n" +
                        "1. file:// - 读取本地文件\n" +
