...
export const linuxPrivEscSkills: AttackDefenseSkill[] = [
    {
        ...
        knowledge: {
            description: 'Linux 权限提升技术用于在获取普通用户访问权限后提升?root。Linux 系统存在多种提权路径：sudo 配置错误、SUID 二进制滥用、危险的 Capability 配置、可?cron 任务、内核漏洞等。提权成功取决于系统配置和用户权限',
            symptoms: [
                '用户可通过 sudo 执行特定命令',
                '系统存在 SUID root 二进制文',
            ...
            observations: [
                'sudo 配置是最常见?Linux 提权路径',
                'GTFOBins 提供了大?SUID 利用技',
                'Docker/LXD 组成员身份等同于 root 访问',
                'Capability 滥用是较新但越来越常见的攻击',
                '内核提权漏洞因版本而异且需?PoC 适配',
            ],
            commonMistakes: [
                '忽略 sudo -l 的输出分',
                '未检查所?SUID 二进',
                '忽略 cron 任务的安全',
                '未检查环境变量劫?(PATH, LD_PRELOAD)',
                '忽略Capabilities 配置',
            ],
            notes: [
                '提权方法高度依赖系统配置',
        ...
        action: {
            checklist: [
                '检?sudo 权限 (sudo -l)',
                '查找 SUID 二进?(find / -perm -4000)',
                '检查用户组 (id)',
                '检?cron 任务 (crontab -l, ls -la /etc/cron*)',
                '检?Capability (getcap -r / 2>/dev/null)',
                '检查可写配置文?(/etc/passwd, /etc/shadow)',
                '检查环境变',
                '检查内核版本和可用 exploit',
                '使用 LinPEAS 自动化枚?',
            ],
            ...
            examples: [
                {
                    name: 'sudo vim 提权',
                    description: '利用 sudo 允许运行 vim 的特权执?shell',
