...
description: "Windows 权限提升技术用于在获取低权?shell 后提升到 SYSTEM ?Administrator 权限 适用于: 获取普通用户权限后需要提升到 SYSTEM ?Administrator; 发现服务配置权限错误; 发现可写系统目录或注册表"
license: MIT
metadata:
...
Windows 权限提升技术用于在获取低权?shell 后提升到 SYSTEM ?Administrator 权限。Windows 系统存在多种提权路径：服务配置错误、可写系统路径、令牌操作、UAC 绕过、内核漏洞利用等。提权成功取决于系统配置、权限和补丁级别
## 何时使用
...
1. 枚举当前用户信息和特?(whoami /all)
2. 检查系统补丁级?(systeminfo)
3. 枚举运行中的服务和权
4. 检查可写系统目录和注册
5. 检?AlwaysInstallElevated 策略
6. 检查服务路径引号问
7. 检查令牌特
8. 检?UAC 配置
