---
name: api-jwt-001
description: "JWT (JSON Web Token) 攻击技术利?JWT 实现中的设计缺陷、配置错误和协议层漏洞，通过修改算法、伪造签名、操纵声明等方式绕过身份验证 适用于: 目标 API 使用 JWT (JSON Web Token) 进行身份认证或授; Authorization: Bearer <token> 请求头中包含 JWT; Cookie 中存?JWT 用于会话管理"
license: MIT
metadata:
...
`jwt`, `json web token`, `bearer token`, `access_token`, `id_token`, `alg`, `none algorithm`, `algorithm confusion`, `key confusion`, `kid`, `jku`, `x5u`, `token manipulation`, `jwt bypass`, `jwt attack`, `token forgery`, `弱密`, `暴力破解`
...
`jwt 伪`, `jwt 绕过`, `token 篡改`, `algorithm confusion attack`, `key confusion`, `hmac confusion`, `jwt 密钥爆破`, `jwt 签名绕过`
