---
name: api-ratelimit-001
description: "速率限制 (Rate Limiting) 绕过技术用于规?API 对请求频率、数量或带宽的限制，使攻击者能够在受限条件下执行大量请求 适用于: 目标 API 对登录、注册、密码重置等接口实施了速率限制; API 返回 429 Too Many Requests 响应; 响应头中包含速率限制信息：X-RateLimit-Limit、X-RateLimit-Remaining、Retry-After"
license: MIT
metadata:
...
速率限制 (Rate Limiting) 绕过技术用于规?API 对请求频率、数量或带宽的限制，使攻击者能够在受限条件下执行大量请求。速率限制通常基于 IP 地址、用户账户、API key 或会话实施。绕过技术包?IP 轮换、请求头伪造、端点变异、时序攻击、并行请求和协议层绕过。成功绕过速率限制可导致暴力破解成功、数据大规模爬取、拒绝服务和 API 滥用。速率限制的实现质量差异很大，很多系统存在可被利用的盲区
## 何时使用
...
`rate limit`, `rate limiting`, `throttling`, `429`, `too many requests`, `retry after`, `rate limit bypass`, `rate limit evasion`, `brute force bypass`, `ip rotation`, `x-forwarded-for`, `user-agent rotation`, `token bucket`, `sliding window`, `api abuse`, `rate limiting bypass`, `速率限制`, `限速绕`, `暴力破解绕过`, `graphql batching`, `批量请求`
...
`rate limit bypass`, `throttle evasion`, `brute force rate limit bypass`, `ip spoofing`, `header injection`, `rate limit evasion`, `限速绕`, `频率限制绕过`, `429 bypass`, `anti-bruteforce bypass`
...
8. 测试并行请求（Race Condition）是否在计数器更新前完成
9. 测试 GraphQL batching/nesting 是否绕过速率限制
10. 测试时间窗口边界（滑动窗?vs 固定窗口
11. 检查速率限制响应头，分析限制规则
12. 尝试 API key 轮换或枚举未使用?API key
...
### X-Forwarded-For 头伪造绕?IP 速率限制

通过伪?X-Forwarded-For 请求头欺骗服务端使用不同?IP 进行速率限制计数
...
同一功能可能有多?API 端点，速率限制可能只应用于部分端点
...
在单?GraphQL 请求中包含多个查?变更，绕过基于请求数的速率限制

```
场景: GraphQL API 限制每分?10 个请?
正常请求:
POST /graphql
...
利用令牌桶或固定窗口算法的时间特性，在特定时间点集中发送请

```
场景: 令牌桶算法，每秒添加 10 个令牌，桶容?10
...
通过枚举或轮?API key 分散速率限制，每?key 使用其独立配
