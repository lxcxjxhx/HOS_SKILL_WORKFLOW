...
exports.isEmptyCatch = isEmptyCatch;
exports.hasErrorPath = hasErrorPath;
exports.isNoIOSystem = isNoIOSystem;
exports.isTooPerfect = isTooPerfect;
exports.detectSilentFailure = detectSilentFailure;
const types_1 = require("../core/types");
const reality_binder_1 = require("./reality-binder");
/** 错误处理关键词 */
 ...
 * 检查 catch 块是否为空（沉默失败）
 */
function isEmptyCatch(lines, catchLineIdx) {
    ...
    for (let i = catchLineIdx; i < Math.min(catchLineIdx + 5, lines.length); i++) {
        ...
        if (trimmed.match(/^\}?\s*catch\s*(?:\([^)]*\))?\s*\{\s*\}$/))
            return true;
        // catch 后直接是 } (空catch)
        if (trimmed.match(/^\}\s*$/) && i === catchLineIdx + 1)
            return true;
 ...
 * 检查函数是否缺乏错误路径
 */
function hasErrorPath(lines, funcStartIdx) {
 ...
 * 检查是否为"无 I/O 的完整系统"
 */
function isNoIOSystem(lines) {
    const ioLines = lines.filter(l => (0, reality_binder_1.hasIO)(l));
    return ioLines.length === 0;
}
/**
 * 检查函数/模块是否"太过完美"（完整但无真实意义）
 */
function isTooPerfect(funcLines, hasRealIO) {
    if (hasRealIO)
        return false;
    // 检查是否有错误处理
    const hasErrorHandling = funcLines.some(l => ERROR_HANDLING_KEYWORDS.some(kw => l.includes(kw)));
    // 如果完整但无 I/O 且无错误处理 → 怀疑是 silence demo
    if (!hasErrorHandling)
        return true;
    // 检查是否只有 catch 但没有实际恢复
    const hasCatch = funcLines.some(l => l.includes('catch'));
    const hasActualIO = funcLines.some(l => (0, reality_binder_1.hasIO)(l));
    return hasCatch && !hasActualIO;
}
/**
 * Layer 4 检测入口
 */
function detectSilentFailure(file, lines, config = types_1.DEFAULT_CONFIG) {
    const findings = [];
    ...
    if (options.checkMissingErrorPath) {
        for (let i = 0; i < lines.length; i++) {
            ...
            if (trimmed.match(/function\s+(get|set)\w+/))
                continue;
            const endIdx = findMatchingBrace(lines, i);
            if (endIdx === -1)
                continue;
            const funcLines = lines.slice(i, endIdx + 1);
            const hasRealIO = funcLines.some(l => (0, reality_binder_1.hasIO)(l));
            // 只有复杂函数（> 5 行）才需要检查
            if (funcLines.length < 5)
                continue;
            if (!hasRealIO)
                continue; // 无 I/O 的纯函数不需要 error path
    ...
    if (options.checkNoIOSystem && lines.length >= 20) {
        const hasAnyIO = lines.some(l => (0, reality_binder_1.hasIO)(l));
        const hasErrorHandling = lines.some(l => ERROR_HANDLING_KEYWORDS.some(kw => l.includes(kw)));
...
//# sourceMappingURL=silent-failure.js.map