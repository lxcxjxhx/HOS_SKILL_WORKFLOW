...
- Java ysoserial 提供了多?gadget chain（CommonsCollections、Spring、JRE 等）
- PHP 7+ ?unserialize 有类型限制（PHP_serialize、PHP_native 模式
- Python pickle 本质?VM 指令集，可执行任意操
- Node.js 原型链污染不直接导致 RCE，但可结合其他漏洞（如模板注入）
- YAML ?yaml.load() ?Python PyYAML < 5.1 默认执行任意代码
- Ruby YAML ?!ruby/object 可实例化任意
- .NET BinaryFormatter 反序列化是最危险?.NET 序列化方
...
- Python pickle 忽略?Python 版本和操作码兼容
