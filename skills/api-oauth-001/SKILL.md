---
name: api-oauth-001
description: "OAuth 2 适用于: 目标应用使用 OAuth 2.0 ?OpenID Connect (OIDC) 进行第三方登录或 API 授权; 存在 \"Login with Google/GitHub/Facebook\" 等社交登录按; API 使用 OAuth access_token ?OIDC id_token 进行认证"
license: MIT
metadata:
...
OAuth 2.0 ?OpenID Connect (OIDC) 是现代应用最广泛使用的授权和认证协议。OAuth 流程攻击利用协议实现中的配置错误、参数校验不严和流程设计缺陷，通过拦截授权码、操纵重定向 URI、绕?CSRF 保护、窃?token 等方式获取未授权访问。OAuth 的安全性依赖于多个安全机制的正确实现：state 参数?CSRF、redirect_uri 严格匹配、PKCE 防授权码拦截、token 安全传输和存储。任何一个环节的疏漏都可能导致认证绕过或 token 泄露
## 何时使用
...
`oauth`, `oauth2`, `openid`, `oidc`, `authorization code`, `access token`, `id token`, `refresh token`, `redirect_uri`, `state parameter`, `pkce`, `code_verifier`, `code_challenge`, `implicit grant`, `client credentials`, `token endpoint`, `authorization endpoint`, `scope`, `consent`, `social login`, `第三方登`, `oauth 攻击`, `token 泄露`, `授权码拦`, `redirect uri 操纵`
...
2. 检?redirect_uri 参数是否接受任意 URL 或仅白名?URL
3. 验证 state 参数是否存在、是否随机、是否在回调时验
4. 检?PKCE 是否实施（code_challenge ?code_verifier
5. 测试授权码是否可被截获并兑换?access_token
6. 检?token 传输方式（HTTPS、是否在 URL fragment 中暴露）
7. 测试 Referer 头是否会泄露 token
8. 检?id_token 签名验证?aud 声明验证
9. 测试 scope 验证是否严格
10. 检?OAuth 客户端密钥是否暴露在客户端代码中
11. 测试动态客户端注册是否存在未审批注册漏
12. 检?token 刷新流程是否安全（refresh token 轮换?
...
截获 OAuth 授权码并在攻击者控制的客户端兑?access_token
...
修改 redirect_uri 参数将授权码?token 发送到攻击者控制的地址
...
OAuth 流程中缺?state 参数验证，攻击者可强制用户使用攻击者账户登
...
```
场景: SPA 使用 Implicit Grant，access_token ?URL fragment 中
...
利用 Implicit Grant 流程?token ?URL fragment 暴露的弱点窃?token
...
请求超出预期?OAuth scope 获取更高权限?access_token
