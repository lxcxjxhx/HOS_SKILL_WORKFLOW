---
name: web-ssrf-001
description: "服务端请求伪?(SSRF) 允许攻击者控制服务器发起?HTTP 请求目标，从而访问内网服务、云平台元数据、本地文件等受限资源 适用于: 应用存在 URL 参数用于获取远程资源（如图片下载、PDF 生成、Webhook 回调; URL 参数控制后端发起 HTTP 请求到外部服; 云环境部署的应用可访问内部元数据服务"
license: MIT
metadata:
...
服务端请求伪?(SSRF) 允许攻击者控制服务器发起?HTTP 请求目标，从而访问内网服务、云平台元数据、本地文件等受限资源。SSRF 的危害在于它绕过了网络层的访问控制——服务器自身拥有内网访问权限，攻击者通过控制请求目标间接获得内网访问能力。在云环境中，SSRF 可访问实例元数据服务获取临时凭证，进而接管整个云账户
## 何时使用
...
2. 测试基础 SSRF：替?URL ?http://attacker.com/ 确认可控
3. 测试本地回环?27.0.0.1, localhost, 0.0.0.0, [::1]
4. 测试 IP 编码绕过：八进制 (0177.0.0.1)、十进制 (2130706433)、十六进?(0x7f000001)
5. 测试 IPv6 绕过：[::1]、[0:0:0:0:0:ffff:127.0.0.1]
6. 测试云元数据服务?69.254.169.254（AWS）?00.100.100.200（阿里云
7. 测试端口扫描：通过响应时间/错误判断内网端口开放状
8. 测试协议处理器：file:///etc/passwd、gopher://、dict://
9. 测试 DNS rebinding：使用工具（?rbndr.us）控?DNS 解析结果
10. 测试重定?SSRF：控制外部服务器重定向到内网地址
...
利用 SSRF 访问 AWS EC2 实例元数据服务获?IAM 临时凭证
...
当应用过?localhost/127.0.0.1 时，使用各种 IP 编码形式绕过

```
常见过滤: url.contains('localhost') || url.contains('127.0.0.1')
...
利用 gopher:// 协议构造任?TCP 请求攻击内网 Redis 服务
...
利用 SSRF 访问阿里?ECS 实例元数据服

```
阿里云元数据端点: http://100.100.100.200/latest/meta-data/
