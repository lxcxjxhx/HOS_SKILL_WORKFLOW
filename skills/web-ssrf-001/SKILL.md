---
name: web-ssrf-001
description: "服务端请求伪造(SSRF) 允许攻击者控制服务器发起 HTTP 请求目标，从而访问内网服务、云平台元数据、本地文件件等受限资源 适用于: 应用存在 URL 参数用于获取远程资源（如图片下载、PDF 生成、Webhook 回调; URL 参数控制后端发起 HTTP 请求到外部服; 云环境部署的应用可访问内部元数据服务"
license: MIT
metadata:
