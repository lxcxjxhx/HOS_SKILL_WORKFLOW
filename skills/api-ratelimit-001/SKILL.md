...
description: "速率限制 (Rate Limiting) 绕过技术用于规避 API 对请求频率、数量或带宽的限制，使攻击者能够在受限条件下执行大量请求 适用于: 目标 API 对登录、注册、密码重置等接口实施了速率限制; API 返回 429 Too Many Requests 响应; 响应头中包含速率限制信息：X-RateLimit-Limit、X-RateLimit-Remaining、Retry-After"
license: MIT
metadata:
...
`rate limit`, `rate limiting`, `throttling`, `429`, `too many requests`, `retry after`, `rate limit bypass`, `rate limit evasion`, `brute force bypass`, `ip rotation`, `x-forwarded-for`, `user-agent rotation`, `token bucket`, `sliding window`, `api abuse`, `rate limiting bypass`, `速率限制`, `限速绕过`, `暴力破解绕过`, `graphql batching`, `批量请求`
...
`rate limit bypass`, `throttle evasion`, `brute force rate limit bypass`, `ip spoofing`, `header injection`, `rate limit evasion`, `限速绕过`, `频率限制绕过`, `429 bypass`, `anti-bruteforce bypass`
