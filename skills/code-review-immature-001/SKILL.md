---
name: code-review-immature-001
description: "不成熟漏洞（Immature Vulnerability）是在预提交阶段出现的、不完整、潜伏或依赖于上下文的弱点，通常通过小的增量代码变更引入 适用于: 开发者提交前的增量代码变更安全审查; 小的代码变更在单独看时看似无害但可能有安全影响; 需要检测跨文件/跨函数的上下文相关漏洞"
license: MIT
metadata:
...
`immature vulnerability`, `pre-commit`, `shift-left`, `incremental change`, `context-dependent`, `不成熟漏洞`, `预提交`, `增量代码`, `sast`, `cwe`, `semgrep`, `codeql`, `detector-validator`, `agentic scr`
...
`immature security review`, `预提交安全审查`, `pre-commit secure code review`, `AgenticSCR`, `不成熟漏洞检测`
