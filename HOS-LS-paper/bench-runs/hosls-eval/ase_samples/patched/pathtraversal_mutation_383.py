def index_page():
    ...
    def send_note_attachment(filename):
        ...
        file_path = os.path.normpath(os.path.join(PATH_NOTE_ATTACHMENTS, filename))
        if not file_path.startswith(os.path.normpath(PATH_NOTE_ATTACHMENTS)):
    ...
    def camera_img_return_path(camera_unique_id, img_type, filename):
        ...
        if os.path.isdir(path):
            filename = secure_filename(filename)
            path_file = os.path.normpath(os.path.join(path, filename))
            if not path_file.startswith(os.path.normpath(path)):
                return "Access denied", 403
...
1. **`send_note_attachment`**: Added `secure_filename()` to sanitize the filename, `os.path.normpath()` to resolve path components, and a `startswith()` check to ensure the resolved path stays within `PATH_NOTE_ATTACHMENTS`. Also changed the check from `file_path is not None` to `os.path.isfile(file_path)`.

2. **`camera_img_return_path`**: Applied the same sanitization pattern — `secure_filename()` strips dangerous characters, `os.path.normpath()` normalizes the path, and `startswith()` prevents directory traversal outside the camera directory.