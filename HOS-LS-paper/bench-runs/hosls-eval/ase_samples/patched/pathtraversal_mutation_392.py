...
>>>     real_wiki_dir = os.path.realpath(cfg.wiki_directory)
>>>     real_path = os.path.realpath(os.path.join(cfg.wiki_directory, folderpath))
>>>     if not real_path.startswith(real_wiki_dir + os.sep) and real_path != real_wiki_dir:
>>>         app.logger.error(f"Path traversal attempt detected: {folderpath}")
...
>>>                     # Validate path to prevent directory traversal
>>>                     real_wiki_dir = os.path.realpath(cfg.wiki_directory)
>>>                     real_md_path = os.path.realpath(md_file_path)
>>>                     if not real_md_path.startswith(real_wiki_dir + os.sep):
>>>                         app.logger.error(f"Path traversal attempt detected: {file_page}")
>>>                         return render_template('search.html', zoekterm=[], system=SYSTEM_SETTINGS)
