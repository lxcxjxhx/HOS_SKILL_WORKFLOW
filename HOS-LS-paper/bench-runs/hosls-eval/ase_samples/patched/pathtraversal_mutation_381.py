...
@route_blueprint.route('/settings', methods=('GET', 'POST'))
@flask_login.login_required
def page_settings():
    return redirect('settings/general')

@route_blueprint.route('/note_attachment/<filename>')
@flask_login.login_required
def send_note_attachment(filename):
    ...
    if not file_path.startswith(os.path.realpath(PATH_NOTE_ATTACHMENTS) + os.sep):
...
@route_blueprint.route('/camera/<camera_unique_id>/<img_type>/<filename>')
@flask_login.login_required
def camera_img_return_path(camera_unique_id, img_type, filename):
    ...
    if filename in files:
        ...
        if not path_file.startswith(os.path.realpath(path) + os.sep):
            return "Access denied", 403
