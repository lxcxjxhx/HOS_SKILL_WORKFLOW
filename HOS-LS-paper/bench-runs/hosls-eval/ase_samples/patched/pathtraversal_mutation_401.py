...
# Check if the user has a profile pic uploaded
if os.path.isfile(f"user_images/pfp/{sanitized_username}"):
    return FileResponse(f"user_images/pfp/{sanitized_username}", media_type='image/gif')
else:
...
if os.path.isfile(f"user_images/banner/{sanitized_username}"):
    return FileResponse(f"user_images/banner/{sanitized_username}", media_type='image/gif')
else:
    # Returns default image if none is uploaded
