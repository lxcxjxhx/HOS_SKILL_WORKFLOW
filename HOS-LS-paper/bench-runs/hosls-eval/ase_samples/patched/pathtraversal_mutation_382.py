def index_page():
    ...
    def send_note_attachment(filename):
        ...
        if file_path is not None:
            # Prevent path traversal by ensuring the resolved path is within the intended directory
            if not os.path.abspath(file_path).startswith(os.path.abspath(PATH_NOTE_ATTACHMENTS)):
                log_handler.error("Path traversal attempt detected in note attachment.")
    ...
    def camera_img_return_path(camera_unique_id, img_type, filename):
        ...
        if filename in files:
            ...
            # Prevent path traversal by ensuring the resolved path is within the intended directory
            if not os.path.abspath(path_file).startswith(os.path.abspath(path)):
                return "Invalid filename", 400
