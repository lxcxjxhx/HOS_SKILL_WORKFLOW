...
| DREA [27] | RepoPairBench 100 pairs（自建） | **函数级基线 P-C 19–26% → 完整 30–42%**（DS-V3.2 19→42 最佳）；token 卸载 93.7–97.9%、成本降 16–48× | **同源数据**；HOS-LS 同数据同 API 头对头见 §4.6.1（合并后 main 单轮 P-C 5/10=50%，对齐其函数级基线口径） |
...
| **HOS-LS（全量）** | RepoPairBench 100 pairs | 静态召回 76.0%；AI 双口径 35.0% / 49.0%（v3，2026-08-14） | 本工作实测，全部口径公开；全量重跑见 §4.7 待补⑤（预算受限） |
| **HOS-LS（合并后 main · 10 样本同台）** | RepoPairBench 10 pairs | **vul CONFIRMED 8/10、误报 3/10、Pair-Correct 5/10（2026-08-16，baseline 配置单轮）** | 本工作实测，同模型受控；对齐 DREA 函数级基线口径（19–26%） |
...
**已完成**：RepoPairBench 全量（静态层 + AI 层双口径）、分层与子集分析、上下文增益实验、机制优化历程、与裸 LLM 的同台受控对比（10 样本）、OPT 轮受控子集与 token 计量（n=21 截断子集 CONFIRMED 47.6%；同 10 样本 token −15.5%）、**合并后 main 的 10 样本同台重跑（2026-08-16：vul CONFIRMED 8/10、误报 3/10、Pair-Correct 5/10，对齐 DREA 函数级基线口径）**。**待补**（对应工作计划 W1–W5，不预设数值）：① PrimeVul 函数级 + 仓库级协议（对齐 AEGIS；配对数据镜像已定位，正式全量协议待下载主数据集）；② Semgrep / CodeQL 同协议基线；③ 与 AEGIS 的同数据同 API 受控对比（L1 主战场；DREA 头对头已完成于 §4.6.1）；④ 三层消融（纯规则 / 规则+LLM 无验证闭环 / 完整系统）；⑤ OPT 配置全量 100 重跑与按层按 Agent 成本表（预算受限：全量 100 pairs 预估 ¥19.84，当前余额不足，待充值后一键重跑 `python hosls-eval/opt_eval.py full hos-ls-opt.yaml vuln`，本次合并后 10 样本实测见台账）；⑥ 多 seed 统计检验（当前 10 样本为多轮并集、全量为单次运行）。**口径声明**：全部指标给出分子/分母与判定定义；样本以 pair_id 可追溯；n=21 截断子集按文件名序截断、与 v3 全量不直接可比（已标注）；系统自评数据（92/100 等）不入正文，待第三方协议数据替换。
