...
**检出质量校准（DREA 自己的证据）**：DREA 论文与其开源仓库明确报告「**26–55% 的真阳性是 Lucky Hits**——标签正确但推理与真实漏洞机制不符」（对 DREA 与函数级基线均成立）。这意味着 DREA 基线的 8/10 `VULNERABLE` 标签中约 2–4 个属于"碰对的标签"；而 HOS-LS 的 CONFIRMED 要求验证链完整（行号/AST 污点/验证状态）才计入。**以"证据完整的检出"为同一质量口径，HOS-LS 的 6/10（全部带证据链）与 DREA 的 8/10（含至多约半数幸运命中）的差距显著收窄乃至反向**——这正是本工作把确定性验证做成默认能力、而非事后评测加分项的原因。
